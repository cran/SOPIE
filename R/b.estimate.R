...
#{
#if (end<=1) lengte<- round(findInterval(end,vela)-start-1)
#else lengte<- (end-start-1)
#}
...
{
if ((endpoint > end) & (end > 1 )) endpoint<-end
else if (endpoint > length(data)) endpoint<-length(data)
}
teller<-1
n<-endpoint-start-1
...
#CVM<-list(rejection=CVM_rejectionpnt,mean=mean(CVM_rejectionpnt),stdev=sqrt(var(CVM_rejectionpnt)),
#minimums=data[close_to_min_index],grow=g,nr_reject=r,alpha=alpha,kernel="1-cos di")

#KS<-list(rejection=KS_rejectionpnt,mean=mean(KS_rejectionpnt),stdev=sqrt(var(KS_rejectionpnt)),
#minimums=data[close_to_min_index],grow=g,nr_reject=r,alpha=alpha,kernel="1-cos di")

#AD<-list(rejection=AD_rejectionpnt,mean=mean(AD_rejectionpnt),stdev=sqrt(var(AD_rejectionpnt)),
#minimums=data[close_to_min_index],grow=g,nr_reject=r,alpha=alpha,kernel="1-cos di")

#rayleigh<-list(rejection=rayleigh_rejectionpnt,mean=mean(rayleigh_rejectionpnt),stdev=sqrt(var(rayleigh_rejectionpnt)),
#minimums=data[close_to_min_index],grow=g,nr_reject=r,alpha=alpha,kernel="1-cos di")
